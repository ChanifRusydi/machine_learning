...
camera_mode = st.selectbox("Camera Mode", ("Single", "Double"))
...
if camera_mode == "Single":
    ...
    with st.container():
        ...
        while True:
            status1, frame = camera.read()
            if status1:
                status, frame = detect(frame)
                camera_placeholder.image(frame, use_column_width=True, channels="BGR")
                if cv2.waitKey(1) & 0xFF == ord("q") or stop_button:
                    break
            else:
                camera_placeholder.write("Camera is not available")
    # with st.container():
    #     camera_placeholder = st.empty()
    #     while True:
    ...
    #         if status:
    #             image = detect(frame)
    #             camera_placeholder.image(image, use_column_width=True, channels="BGR")
    #             if cv2.waitKey(1) & 0xFF == ord("q") or stop_button:
    #                 break
    #         else:
    #             camera_placeholder.write("Camera is not available")
            ...
            if status1 and status2:
                ...
                if frame1 is None or frame2 is None:
                    ...
                    if frame1.shape != frame2.shape:
                        side_by_side_placeholder.subheader("Please open camera with same resolution")
                    else:
                        stitching_status, image = image_stitching(frame1, frame2)
                        if stitching_status == -1:
