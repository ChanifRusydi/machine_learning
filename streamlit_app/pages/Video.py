...
mode = st.selectbox("Select Mode", ["One Video", "Two Video"])
                

video_status1 = False
video_status2 = False

def upload_two_video():
    with st.container():
...
if mode == "One Video":
    ...
    video_upload = video_placeholder.file_uploader("Upload Video", type=['mp4'])
    if video_upload is not None:
        with open('video.mp4', 'wb') as f:
            f.write(video_upload.getbuffer())
            
        open_video = cv2.VideoCapture('video.mp4')
        while open_video.isOpened():
            ret, frame = open_video.read()
            if not ret:
                break
            frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
            status,image = detect(frame)
