...
class WandbLogger():
    def __init__(self, opt, name, run_id, data_dict, job_type='Training'):
        ...
        if isinstance(opt.resume, str):  # checks resume from artifact
            ...
            # project_name ='YOLOv7 Traffic' if opt.project == 'runs/train/YOLOv7 Traffic' else 'YOLOR' if opt.project=='runs/train' else Path(opt.project).stem
            self.wandb_run = wandb.init(config=opt,
                                        resume="allow",
                                        project='YOLOv7 Traffic' if opt.project == 'runs/train/YOLOv7 Traffic' else 'YOLOR' if opt.project=='runs/train' else Path(opt.project).stem,
                                        name=name,
                                        job_type=job_type,
