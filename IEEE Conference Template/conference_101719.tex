...
Best result of 15 frame per second in traffic object detection inference process is achieved when the edge computer is using 10 Watt of power.
\end{abstract}
...
Despite its popularity,the COCO Dataset has some drawbacks. Although COCO Dataset contains quite large number of image classes. However,the COCO Dataset has an imbalanced class distribution. 
For example, as shown in Fig.\ref{fig:original_coco_classes_count} the number of person class is 64115, while the number of hair drier class is only around 100. Also, for this research, the model doesnt need to detect anything other than objet that relate to traffic
\begin{figure}[!h]
\centering
\includegraphics[width=0.5\textwidth,keepaspectratio]{coco_class_count.png}
\label{fig:original_coco_classes_count}
