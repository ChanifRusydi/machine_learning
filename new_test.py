...
def image_stitching(image1, image2):
    ...
    for idx, name in enumerate(image_names):
        ...
        print(dst_sz)
        print('image warped size', image_warped_s.shape)
        print('mask warped size', mask_warped.shape)
        cv2.imshow('image warped', image_warped_s)
        cv2.imshow('mask warped', mask_warped)
        cv2.waitKey(0)
        print('corner', corners[idx])
...
if __name__ == "__main__":
    image1 = cv2.imread('images/image1_60_left.jpg')
    image2 = cv2.imread('images/image1_60_right.jpg')
